...
void mp_nextprime(integer_class &res, const integer_class &i)
{
	...
	while (!mp_probab_prime_p(candidate, 25)) {candidate += 2;}
...
struct two_by_two_matrix {
	...
	two_by_two_matrix operator *(const two_by_two_matrix &other) {
		...
		res.data[0][0] = this->data[0][0]*other.data[0][0] + this->data[0][1]*other.data[1][0];
		res.data[0][1] = this->data[0][0]*other.data[0][1] + this->data[0][1]*other.data[1][1];
		res.data[1][0] = this->data[1][0]*other.data[0][0] + this->data[1][1]*other.data[1][0];
		res.data[1][1] = this->data[1][0]*other.data[0][1] + this->data[1][1]*other.data[1][1];
		return res;
	}
	//recursive repeated squaring
	two_by_two_matrix pow(unsigned long n) {
		if (n == 0) {return two_by_two_matrix::identity();}
		if (n == 1) {return *this;}
		if (n == 2) {return (*this)*(*this);}
		if (n % 2 == 0) {return (this->pow(n/2)).pow(2);}
...
void mp_fac_ui(integer_class &res, unsigned long n) {
	...
	res = 1;
	for (unsigned long i = 2; i <=n; ++i) {res *= i;}
}
...
bool mp_perfect_power_p(const integer_class &i) 
{
	...
	while (true) {
		...
		if (mp_root(root, i, p.convert_to<unsigned long>())) {return true;}
	}
}
...
int unchecked_jacobi(const integer_class &a, const integer_class &n) {
	...
	if (num == 1) {return product_of_twos;}
	if (boost::multiprecision::gcd(num,den) != 1) {return 0;}
	...
	if ((fmod(num,4) == 3) && (fmod(den,4) == 3)) {quadratic_reciprocity_factor = -1;}
	return product_of_twos * quadratic_reciprocity_factor * unchecked_jacobi(den, num);
}

// public interface for computing jacobi symbols.  performs checking.
int mp_jacobi(const integer_class &a, const integer_class &n)
{
	if (n < 0) {throw std::runtime_error("jacobi denominator must be positive");}
	if (n % 2 == 0) {throw std::runtime_error("jacobi denominator must be odd");}
...
int mp_kronecker(const integer_class &a, const integer_class &n)
{
	...
	(a | 2) is 0 if a is even, 1 if (a mod 8) == 1 or 7, and -1 if (a mod 8) == 3 or 5.
	...
	PRODUCT [(a | p_i)**e_i]
	== (a | 2)**j * PRODUCT [(a | p_i)**e_i]	here the p_i's are the remaining (odd) prime factors
	== (a | 2)**j * (a | m), 					here (a | m) is the Jacobi symbol, because m>0 is odd
	...
	*/
	...
	if (n == 0) {throw std::runtime_error("second arg of Kronecker cannot be zero");}

	//Compute (a | u)
	int kr_a_u = 1;
	if (n.sign() == -1 && a<0) {kr_a_u  = -1;}
	...
	if (a % 2 != 0) {
		...
		kr_a_2_to_j = (kr_a_2 == -1 && (j % 2 != 0)) ? -1 : 1; //(-1)**odd == -1; (-1)**even=(1)**integer=1
	}
